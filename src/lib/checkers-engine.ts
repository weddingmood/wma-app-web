...
export function createInitialCheckersState(
  ...
  return {
    ...
    gameMode,
    aiLevel,
    lastMessage: "La partie de Dames commence. À David d'ouvrir le jeu !",
  };
}
...
export function executeCheckersMove(
  ...
  let msg = `${turn === "partner1" ? "David" : "Ruth"} a joué son coup.`;

  if (capturedCount > 0) {
    msg = `${turn === "partner1" ? "David" : "Ruth"} a capturé une pièce adverse !`;
  }
  if (becameKing) {
    msg = `Couronnement ! La pièce de ${turn === "partner1" ? "David" : "Ruth"} devient Dame.`;
  }
  ...
  if (newPiecesCount[nextTurn] === 0 || oppMoves.length === 0) {
    status = "finished";
    winner = turn;
    msg = `Victoire éclatante de ${turn === "partner1" ? "David" : "Ruth"} au jeu de Dames !`;
  }
